...
function parseVariableData(lines, comments){
	...
	var ownerStack = []; //saves owner {name}
																													//"name" is never undefined
																													//either "data" or "array" can be undefined
																													//array recursively saves same object
																													//when saving into, if curly bracket level increases use "array" (even if there's only one entry)
																													//when saving into, if there are no curly brackets use "data"
																													//when reading from, if "array" is defined, push current to stack and after "array" of current is searched trough, pop
																													//when reading from, if "data" is defined, add variable and go to next entry in parent array
																													//leaf objects will have their "data" defined and "array" undefined
	// NOTE use stack for owner name:
	// NOTE [struct0, struct1, struct2] would result in e.g. struct0.struct1.struct2.fields_of_struct2
	// newly found variables would be instantly added to "variables" array and returned

	// variable name, name1, name2;
	// variable name = value;
	// variable name = value, name1 = value1, name2;
	// variable name = struct { property, property=value, property=[array, array], property={structure} };
	for (let i in lines){
		...
		if (foundName == ""){
			foundName = lines[i];
			curlyFoundLevel = curlyLevel;
			continue;
		}
		if (curlyLevel == curlyFoundLevel && squareLevel == 0){
			if (lines[i] == ","){
				variables.push(new slang_main.slang_variable(foundName, "Type_Basic", foundData+" "+comments));
				foundData = "";
				foundName = "";
				comments = "";
				bStartedData = false;
...
//Type_Basic should be parsed in separate function because there'll be a lot of code
