...
export default {
	...
	parseFile_onSave() {
		...
		// Suggestions are shown when this files is changed
		// 	display members of struct (variables or function) when writing "." (or "->"?)
		...
		// //result: ello w
		// var str = "Hello world!";
    	// var res = str.slice(1, str.lastIndexOf("o"));

		// atom.notifications.onDidAddNotification(function () {console.log('File saved - parsing');});
		/* //works:
		let editor;
...
function processFile(file, data) {
	slang_parse.slang_parser.parseData(file, data);
	// slang_parser.parseData(file, data);
}
