...
8 searching for "}" or ","
<<< if found "data" at 6; if found "}" at 5 or 6 search until curly level is zero
	...
	*/
	...
	for (let i in lines){
		...
		switch(state){
			...
			// >>> to 7 if found "="
				if (lines[i] === ","){
					variables.push(new slang_main.slang_variable(ownerStack[ownerStack.length-1]+"."+foundName, struc, comments));
					foundName = "";
					foundData = "";
					state = 5;
				}
			...
			// >>> to 5 if found "," (add "name" and "data" to variables)
				if (lines[i] === "}"){
					curlyLevel--;
					variables.push(new slang_main.slang_variable(ownerStack[ownerStack.length-1]+"."+foundName, struc, foundData+"; "+comments));
					foundName = "";
					foundData = "";
					ownerStack.pop();
					if (curlyLevel <= 0){
